...
function getAllSIMs() {
  ...
  while (true) {
    const response = callSimControlAPI('/sims', {
      page,
      page_size: 100,
      status: 'ALL'  // 👈 includes MANAGED, PAUSED, STAGED, and SUSPENDED SIMs
    });
  ...
  logToSheet('INFO', `✅ Total SIMs fetched (status=ALL): ${sims.length}`);
  return sims;
}
...
function onOpen() {
  ...
  ui.createMenu('🧠 SIMcontrol Tools')
    ...
    .addItem("📶 Update '[API] SimControl Data' on demand", "fetchAllSIMDetailsToSheet")
    //.addItem("📶 Update SIM Descriptions from Sheet (CCID - colD, Desc - colY)", "updateSimDescriptionsFromSheet")
    .addSeparator()
    ...
    .addItem('🧪 Test SIM description update', 'testUpdateSimDescription')
...
function fetchAllSIMDetailsToSheet() {
  const sheetName = '[API] SimControl Data';
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  let sheet = ss.getSheetByName(sheetName);
  ...
  // Step 1: Get all SIMs
  const sims = getAllSIMs(); // Leverages your existing function
  if (!sims || sims.length === 0) {
    logToSheet('ERROR', 'No SIMs retrieved in fetchAllSIMDetailsToSheet');
    return;
  }

  // Step 2: Determine all possible fields from first SIM
  const headers = Object.keys(sims[0] || {});
  if (headers.length === 0) {
    logToSheet('ERROR', 'No SIM fields found.');
    return;
  }

  sheet.appendRow(headers);

  // Step 3: Write SIM data
  const rows = sims.map(sim => headers.map(h => {
    const val = sim[h];
  ...
  SpreadsheetApp.getActiveSpreadsheet().toast(`✅ ${rows.length} SIMs written to sheet`, 'Done');
  logToSheet('INFO', `✅ Fetched and wrote ${rows.length} SIMs to ${sheetName}`);
}
